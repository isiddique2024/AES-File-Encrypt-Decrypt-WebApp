...
async def encrypt_file(request: Request, param: models.encrypt_base):
    ...
    try:
        ...
        encrypt_obj = AES.new(key, param.block_cipher_mode, iv)  # AES.MODE_CBC (2)
        file_encrypted = iv + encrypt_obj.encrypt(
            pad(data_to_encrypt.encode("utf-8"), block_size)
        )
    ...
    return {"file_encrypted": file_b64_return, "key": key_hex, "iv": iv_hex}
...
async def decrypt_file(request: Request, param: models.decrypt_base):
    ...
    if utils.check_file_greater_than_3mb(param.file_encrypted):
        raise HTTPException(
            status_code=400, detail="File size must not be greater than 3mb"
        )

    if len(param.key) not in utils.valid_key_lengths:
        raise HTTPException(
            status_code=400, detail="Key length must be 32, 48, or 64 characters"
        )

    if len(param.iv) != 32:
        raise HTTPException(
            status_code=400, detail="IV length must be exactly 32 characters"
        )
    ...
    try:
        ...
        decrypt_obj = AES.new(key, param.block_cipher_mode, iv)
    ...
    return {"file_decrypted": file_b64_return}
