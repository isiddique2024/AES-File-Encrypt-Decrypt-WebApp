...
const DecryptPage = () => {
    ...
    const handleDecrypt = async (event) => {
        ...
        if ((key.value.length !== 32) && (key.value.length !== 48) && (key.value.length !== 64)) {
        ...
        const byte_array = new Uint8Array(await fileDataDecrypt.value.arrayBuffer());
        if(byte_array){

            const hex = Array.from(byte_array).map((b) => b.toString(16).padStart(2, "0")).join("");
            ...
            const decrypted_file = Uint8Array.from(Buffer.from(response.data.file_decrypted, 'hex'));
            const download = new Blob([decrypted_file], { type: "application/octet-stream;charset=utf-8" });
            ...
            <form onSubmit={handleDecrypt}>
                <div className="mb-4 flex flex-col w-72">
                    ...
                    <div className="form-group flex flex-col items-center">
                        <label className="label mt-4" htmlFor="iv">Initialization Vector (IV)</label>
                        <input
                            type="text"
                            id="iv"
                            placeholder="Type here"
